...
def rnn_backward(dh, cache):
    ...
    # pass
    N, T, H = dh.shape
    x, prev_h, next_h, Wx, Wh, b = cache[0]
    D = x.shape[1]
    ...
    for t in range(T-1, -1, -1):
        # dh_t from output
        # dnext_h from hidden state
        dh_t = dh[:, t, :]
        dx_t, dnext_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh_t+dnext_h, cache[t])
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (next_c, o, f, prev_c, g, i, a_i, a_f, a_o, a_g, Wx, x, Wh, prev_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # pass
    next_c, o, f, prev_c, g, i, a_i, a_f, a_o, a_g, Wx, x, Wh, prev_h = cache
    # backward 
    # next_h = o * np.tanh(next_c)
    ...
    # backward a = x.dot(Wx) + prev_h.dot(Wh) + b
    dx = da.dot(Wx.T)
    dWx = x.T.dot(da)
...
def lstm_backward(dh, cache):
    ...
    _, _, _, _, _, _, _, _, _, _, _, x_0, _, _ = cache[0]
